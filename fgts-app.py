...
def process_file(file_path):
    df_competencias = processar_arquivo(file_path)
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
            df_competencias.to_excel(writer, sheet_name='Geral', index=False)
            df_competencias[["NOME DO TRABALHADOR", "MATRICULA", "COMPETENCIAS_VALIDAS"]].to_excel(writer, sheet_name='Comp_Validas', index=False)
            df_competencias[["NOME DO TRABALHADOR", "MATRICULA", "COMPETENCIAS_NAO_PAGAS"]].to_excel(writer, sheet_name='Nao_Pagas', index=False)
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        sys.exit("Uso: script.py <arquivo.txt>")
    file_path = sys.argv[1]
    try:
        zip_path = process_file(file_path)
        print(zip_path)
