...
def processar_arquivo(txt_path):
    ...
    for bloco in blocos:
        ...
        if nome and matricula and dtadm:
            competencias_ordenadas = ordenar_competencias(competencias.split())
            ...
            for competencia, valor in depositos_atraso:
    ...
    df_aparicoes = pd.DataFrame(lista_aparicoes)
    ...
    if not df_aparicoes.empty:
        df_aparicoes = df_aparicoes.sort_values(by="NOME DO TRABALHADOR")

    return df_competencias, df_depositos, df_aparicoes

def comparar_planilhas(df1_comp, df2_comp, df1_dep, df2_dep):
    # Converter competências para conjuntos
    ...
    # Processar competências comuns
    for (matricula, dtadm) in matriculas_dtadm_comuns:
        linhas1 = df1_comp[(df1_comp['MATRICULA'] == matricula) & (df1_comp['DTA.ADM.'] == dtadm)]
        linhas2 = df2_comp[(df2_comp['MATRICULA'] == matricula) & (df2_comp['DTA.ADM.'] == dtadm)]
        
        sets1 = [set(l.split()) for l in linhas1['COMPETENCIAS_NAO_LOCALIZADAS'] if l]
        sets2 = [set(l.split()) for l in linhas2['COMPETENCIAS_NAO_LOCALIZADAS'] if l]
        
        if not sets1 or not sets2:
            continue
        
        competencias_intersec_1 = set.intersection(*sets1)
        competencias_intersec_2 = set.intersection(*sets2)
        
        # Agora calcula a interseção entre os dois arquivos
        competencias_comuns = competencias_intersec_1 & competencias_intersec_2
        
        if competencias_comuns:
            base = linhas1.iloc[0]
            
            comp_resultados.append({
                'NOME DO TRABALHADOR': base['NOME DO TRABALHADOR'],
                'MATRICULA': matricula,
                'PIS/PASEP': base['PIS/PASEP'],
                'DTA.ADM.': dtadm,
                'COMPETENCIAS_ARQUIVO_1': ' '.join(ordenar_competencias(competencias_intersec_1)),
                'COMPETENCIAS_ARQUIVO_2': ' '.join(ordenar_competencias(competencias_intersec_2)),
    ...
    # Processar depósitos para matrículas comuns
    df1_dep_filtrado = df1_dep[df1_dep.apply(lambda x: (x['MATRICULA'], x['DTA.ADM.']) in matriculas_dtadm_comuns, axis=1)]
    df2_dep_filtrado = df2_dep[df2_dep.apply(lambda x: (x['MATRICULA'], x['DTA.ADM.']) in matriculas_dtadm_comuns, axis=1)]
    
    for _, row in df1_dep_filtrado.iterrows():
    ...
    for _, row in df2_dep_filtrado.iterrows():
        dep_resultados.append({
            'NOME DO TRABALHADOR': row['NOME DO TRABALHADOR'],
            'MATRICULA': row['MATRICULA'],
            'PIS/PASEP': row['PIS/PASEP'],
    ...
    df_resultado_dep = pd.DataFrame(dep_resultados)
    if not df_resultado_dep.empty:
        df_resultado_dep = df_resultado_dep.drop_duplicates(subset=['MATRICULA', 'DTA.ADM.', 'COMPETENCIA', 'VALOR'])
        print(f"Total de depósitos encontrados: {len(df_resultado_dep)}")
    else:
        print("Nenhum depósito encontrado para as matrículas com data de admissão comuns.")
    
    return df_resultado_comp, df_resultado_dep
...
def process_files(file1_path, file2_path):
    ...
    df1_comp, df1_dep, _ = processar_arquivo(file1_path)
    df2_comp, df2_dep, _ = processar_arquivo(file2_path)

    # Comparar competências e depósitos
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        if not df_resultado_comp.empty:
            comp_buffer = BytesIO()
            ...
            comp_buffer.seek(0)
            zip_file.writestr('competencias_nao_localizadas.xlsx', comp_buffer.read())

        # Planilha de depósitos
        if not df_resultado_dep.empty:
            dep_buffer = BytesIO()
            ...
            dep_buffer.seek(0)
            zip_file.writestr('depositos_em_atraso.xlsx', dep_buffer.read())
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Uso: python script.py arquivo1.txt arquivo2.txt", file=sys.stderr)
        sys.exit(1)
